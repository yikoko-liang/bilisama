...
- **语音门在实盘失灵，原因未定（2026-09-09 新开）**：同一后端、同一模型、同一份推下去的指令，脚本验收
  DashScope 21/22，实盘直播 Mock 250 轮只拦下 1 条。已用五组对照排除后端、模型、动态尾段、弹幕回复混入
  历史、连续说话无停顿、上行电平。所有对照送的都是 `say` 合成音频加 2.5 秒**全零**静音，实盘送的是标签页
  混音、永不为零——这是唯一没能变的变量。本轮加了 `dev-talk --record-uplink` 和
  `voice_gate_acceptance --replay`（用法见 runbook），下一步用真实录音回放定位。
