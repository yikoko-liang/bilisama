...
| 90 DANMAKU_SUMMARY | 主播委托的弹幕总结 | 语音式交付，仅次于主播语音 |
...
- **验收集在 2026-09-09 重建**（`tools/voice_gate_acceptance.py`）：旧版 7 条正样本全部显式点名「豆腐」、
  ...
  新旧提示词的交错 A/B（10 轮 20 次运行，同窗口交替）：总判对 72.3% 对 78.2%（配对符号检验 p=0.125，不显著）；
  弱线索碎句 77.1% 对 **100%**（9 好 0 差 1 平，**p=0.0039**）。旧版最差一轮弱线索 1/7，新版十轮全部 7/7、
  零方差。总分不显著是因为新版每轮固定错同样三句（点名的直接提问、承接的选择题、没点名的祈使请求），
  封顶 17~18/22——那三句她的备注证明判断是对的，选错了分支，属于下一轮要单独攻的一类。
...
| 上下文管理 | 静态前缀加动态尾段、变了才推、语音规则和事件规则分作用域、旁路回复镜像写回历史、主播弹幕当共享上下文；她自起的回复说完的话和不接时的场景备注都进冷场话题的对话列表（2026-09-09）；主播委托总结弹幕单独记录边界，从语音轮次开始前尚未处理的最新候选中挑一个热议主题 | 侧路调用不复用直播会话前缀缓存（#71，本质是架构决定）；主播说过什么在 s2s 产品路径上只剩十字备注 |
...
- 语音门（2026-09-09，3.6）留下的三条：记号回复三家都会留在服务端历史里，会不会让她越来越倾向报「不是对
  ...
  探测与判对率验收还没跑（#99，当天已跑，三家 91%~95%）。
- **语音门在实盘失灵：病根在模型的寻址判断，不在记号也不在指令管道（2026-09-09 查清）**
  症状：同一后端、同一模型、同一份指令，脚本验收 DashScope 21/22，实盘 26%~49% 且一场之内剧烈震荡。

  **结论**：记号是那个判断的**读数**，不是判断的**因**。她在弱线索输入（游戏解说的碎句：
  「在这里」「上去」「有」）上会把主播的自言自语判成「在跟我说话」，于是正经回答；
  记号写不写只是这个判断翻面之后留下的痕迹。判别实验（全覆盖合同下把两者拆开）：
  判断错但记号合规 4/4 次运行出错，判断错且记号缺 2/4，判断对且记号合规 1/3、逐轮 1/24——
  记号在不在几乎不改结果，判断对不对改结果。

  **线索强度是主导变量**，这解释了脚本和实盘的落差：同一份合同、同一个探针，
  线索充足的验收台词（带「家人们」「谢谢小明送的火箭」「豆腐」）8/8 次运行干净、误出声 0/64；
  换成游戏解说碎句，7 次里 5 次崩、误出声 37/70。

  **全覆盖合同（每条都写记号，被搭话写 `[TO_ME]`）不是解法**：模型完全认这份合同
  （98/98 合规），但它把「答话」从需要主动放弃记号的低频路径变成菜单里平权的一项，
  弱线索池上四种写法合计 **29/29 次运行崩**，比现行合同（零毒 1/8）更糟。
  另外它在产品上也走不通——`[TO_ME]` 会被 TTS 念出来，四家都是音画一体或文本驱动合成。

  **已推翻、不要再捡**：事件回复替换 base 指令（`dose.py` 复跑 4 次未复现；且实盘那场
  `speak_off` 关掉了弹幕回复，根本没有弹幕回复）；历史累积形成单调反馈（逃逸概率是平的，
  k=1..30 稳定在 6%~15%；同一份历史相邻两窗一个 0/34 一个 32/39）；上行音频、电平、
  VAD 静音、台词长度、连续说话（真实录音在干净会话回放拦截率 99%）。

  **仍未解释**：双稳态和那条平坦的逃逸概率；`voice_responses.md` 第 3~5 行把
  「主播在跟她说话」写成既定事实、与寻址合同的「默认不是」直接矛盾——这条**证据不足以定论**
  （删掉后 3/8 仍崩，只排除了「它是全部原因」，排不掉「它是主因之一」；对照设计的检验力
  只有 8.8%）。

  **两个方法论教训，后续实验必须遵守**：
  1. 端点工作点会漂——同一脚本的运行级崩塌率一小时内从 31% 漂到 100%，8 次一组的对照
     架在动的地板上，跨时段的数字不能直接比。
  2. 临时 harness 里的「拦截率」大半在量取消掉的轮次：门对非 COMPLETED 的回复不走判定
     （`voice_turn.py:248-251`），`MarkerHead` 对空文本返回 `PLAIN`
     （`turn_protocol.py:144`），于是空回复和断在记号中间的残记号都被记成「漏接」。
     健康运行里 81% 的「漏」压根没出声。量漏接必须看音频帧，不能只看有没有被拦。
- **火山 SC2.0 把思维链标记漏进文字通道（2026-09-09 新开）**：回复开头形如
  `<eocot>确实，地龙这血量顶不住啊。`。`MarkerHead` 见到 `<` 即判 `plain`，所以只要有这个前缀，
  她就算写了记号门也看不见——拿她的原话验过。DashScope 无此现象。解析器该容忍开头的 ASCII 控制标记。
