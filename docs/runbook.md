...
普通启动默认使用 `medium` 话痨程度和 `low` 回复长度；直播间 ID 留空，助手默认称呼
主播为“主播”。需要接入真实直播间时，再到“系统 → 直播间配置”填写房间号并连接。
...
四个快捷按钮里，语音输入和语音播报会写回当前配置档并立即生效，暂停是本场临时总闸，设置
按钮只控制面板开关：
...
系统页里的用户配置都不要求手动重启。语音输入、语音播报、环境噪音灵敏度、事件回复开关、
房间号、主播昵称、直播主题简介、话痨程度、回复长度、弹幕整理间隔、电池礼物门槛和进房欢迎
人群会在校验通过后立即应用，并写回 `config/bilisama.toml` 当前启用的 profile。主播昵称点输入框
右侧的「保存昵称」，直播主题简介点下方的「保存直播主题」；输入过程中不会拿半截文字刷新模型，
保存后下一次回复起生效。
写盘或热更新失败时会回滚原值，
...
刷新模型上下文并写回配置，不需要重启；也可以直接改 toml，
这种手工编辑仍要等下次启动读取：
...
直播 Mock 检测成功后，它填写的房间号、直播主题简介和五类事件开关会覆盖系统页配置并写回
当前 profile；换了房间但没有新主题时，简介会清空，系统页旧房间的事件流水也会清空。旧房间
