...
改配置的回执分两种：**「已保存并生效」**是落了盘（保留注释；有 active profile 时写的是
`config/profiles/<名字>.toml`，出厂 normal 档就写 `profiles/normal.toml`）；**「本场已生效，重启恢复默认」**是会话级开关（音频双开关、房间号、
主播昵称），本来就不落盘。改坏了值会自己弹回。
...
- **皮肤**：点卡即换，桌宠当场重挂载，不用确认也不用重启。清单来自两处：随包的
  ...
  `config/profiles/normal.toml`，重启保持。命令行 `--skin` 仍是临时试穿的路。
