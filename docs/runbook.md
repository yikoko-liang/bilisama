...
- **声音设备区**：麦克风和扬声器按设备名选、一键试音（从原「现场」页搬来）。
  ...
  起播缓冲防网络抖动把词切碎；另有一个每 3 秒的巡检在默认输出流挂掉时原地重开它
  （显式 `--output-device` 则钉住不动）。**它跟不了真正的设备热插拔**：PortAudio 的
  设备表和默认设备在进程启动时就冻结了（2026-08-29 反汇编核实，台账 #90），启动后
  才连上的蓝牙耳机它看不见——要换新设备得重启。健康计数在高级页健康卡的
  `audio_output` 一张里。
