...
日志 / 测试 / 配置五个页签，顶栏有紧急闭麦按钮）。面板里的注入框和终端打字完全等价，
`/sc` `/gift` 语法通用。点击形象有动画反应，偶尔她还会开口回一句（15 秒冷却，
说话权闸门空闲才说，不会打断正事）。

配置页不只是看：**亮着的控件直播中就能改**——各事件的说话开关、日志级别、日志记录
弹幕正文。改了立即生效但只管本场，重启回到 toml 的值；每次修改在对话页留一行
「配置已改」的回执，改坏了值会自己弹回。灰行带着「重启生效 / 重连生效 / 重启引擎」
的徽章——这些字段的消费方在启动时就把值拿走了，中途改了也没人读，所以面板诚实地
不给你改（哪些字段能亮是审计过消费方代码定的，不是拍脑袋）。密钥永远不进面板。
...
同一套。界面只把语音输入从电脑麦克风切到共享浏览器的音轨，并按你填的房间号临时连接
`BilibiliEventSource`。共享画面只用于确认选对了页面；当前音频模型看不懂画面内容。

先确认 `config/bilisama.toml` 的 `[room] room_id = 0`，启动时也不要带 `--room`。房间已经
由启动参数固定时，界面不会偷偷再连第二个房间，而是明确提示重启。照常启动：
...
1. 打开桌面面板「现场」，点「直播 Mock → 打开」，macOS 会直接用 Chrome 打开。也可以给
   启动命令加 `--open`，在浏览器页面里进入。要稳定捕获单个 B 站标签页的声音，请用 Chrome。
2. 点「选择共享源」，选正在播放直播的浏览器标签页，并勾选「共享标签页音频」。页面必须
   同时看到画面预览和「音轨已收到」；只有画面、没有音轨不能通过检测。
3. 输入 B 站房间号。短号可以，检测成功后会显示解析出的真实房间号和当前人气。
4. 在「参与伴播的真实事件」里选择哪些事件可以触发说话。关闭某项只阻止它触发回复，
   原始事件仍进入记忆与监控。
...
- 「功能验收」18 条，覆盖语音连续对话、打断、键盘噪声、BGM、弹幕选择、进房欢迎、
  礼物、SC、上舰、冷场、安全、观众记忆、优先级和控制开关；十类直播事件全部覆盖。
...
记录本轮人工判断。语音、打断、噪声和紧急闭麦用例还需要按“你要做”配合说话或点按钮。
