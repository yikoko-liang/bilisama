...
### macOS 一键启动（DashScope + 桌面 UI）

首次准备仍按「环境」一节完成 `uv sync`、`path.sh` 和 `desktop/preview` 的 `npm install`。
之后直接在 Finder 双击仓库根目录的 `启动 BiliSama.command`，不用再手敲后端命令。
它调用同目录的 `start_bilisama.sh`，自动完成这些事：

- 加载 `path.sh`，检查 DashScope 凭据但不打印密钥；
- 自动查找 MacBook 内置麦克风，不跟随系统默认值误选蓝牙麦克风；
- 用 `qwen-audio-3.0-realtime-flash` 启动 director 和桌面 UI；
- 后端已在运行时不重复启动，只补拉被关掉的桌面 UI。

普通启动默认打开麦克风和语音播报，使用 `medium` 话痨程度和 `low` 回复长度；直播间 ID、
主播昵称输入框留空，模型内部把空昵称按“主播”处理。需要接入真实直播间时，再到
“系统 → 直播间配置”填写房间号并连接。这四项是本场状态，下次启动恢复上述默认值。

PortAudio 会在进程启动时绑定当时的输入、输出设备。使用 AirPods 时先连接耳机再启动；
运行中连接或断开耳机后若出现“模型已有回复但听不到声音”，需要重启这次进程，让音频流
重新绑定当前设备。

需要临时指定别的输入设备时，从终端这样启动；编号用
`.venv/bin/python -m sounddevice` 查看：

```bash
BILISAMA_INPUT_DEVICE=<编号> ./start_bilisama.sh
```

要直接连真实直播间，带房间号启动；如果后端已经在跑，先在它的终端按 `Ctrl-C`，房间号只在
新会话启动时生效：
...
排查启动环境但不真正连接模型时运行 `./start_bilisama.sh --check`。这个入口不改
`bilisama.toml`；关停方式仍是在它打开的终端窗口按 `Ctrl-C`，让下播蒸馏正常收尾。
...
- **终端打字＝模拟观众**：直接打字是弹幕；`阿强:内容` 指定观众名（同名同记忆行）；
  `/sc 阿强 30 主播玩什么` 是 SC；`/gift 老板 52` 是 52 电池礼物。装了 prompt_toolkit
...
浏览器打开就能看：她的形象、说话时的文字气泡，以及形象右侧竖排的语音输入、语音播报、
暂停和设置四个按钮。设置按钮打开固定左栏的五页控制中心：系统、聊天记录、助手市场、测试、
运行日志。系统页的「手动事件 Mock」默认折叠；展开后可用 `/sc` `/gift`，注入内容会像真实
房间事件一样经过弹幕窗口、礼物聚合、优先级和回复开关，不受直播间是否已连接影响。终端打字
仍是快速直通测试，不经过人群筛选窗口。

四个快捷按钮里，语音输入和语音播报会立即生效但只管本场，暂停是本场临时总闸，设置按钮只
控制面板开关：

- 语音输入默认打开。关掉后，麦克风或直播 Mock 的浏览器音轨会被等长静音帧替代，模型听不到
  用户说话；真实或 Mock 的弹幕、礼物、进房等直播事件仍会照常触发。
- 语音播报默认打开。关掉时会立即清掉尚未播完的音频，此后的回复只显示文字气泡；再次打开
  后恢复语音播报。
- 未指定 `--output-device` 时，桌面版每秒检查一次 macOS 当前默认输出。运行中连接 AirPods、
  切换系统扬声器或设备短暂掉线后，会自动重建 PortAudio 输出流；终端出现「默认扬声器已切换」
  即表示后续语音已经跟到新设备，不用重启伴播。显式指定设备编号时仍固定使用该设备。
- 麦克风和播报都打开时，桌宠气泡只显示礼物、SC、VIP 进房的回复；主播语音、普通弹幕和
  主动话题不冒泡，避免遮挡直播画面。麦克风关闭或语音播报关闭时，所有回复都用气泡补足。
- 暂停默认关闭。打开后先截断当前回复并清空待播队列，再关闭所有音频和直播事件入口，并主动
  关闭语音 WebSocket，暂停期间不保活也不自动重连。恢复时先新建语音会话并重新推送人设，成功
  后才按暂停前保存的语音输入、语音播报配置开放输入；恢复失败会保持暂停并显示可操作的错误，
  不需要手动重启进程。
- 设置按钮默认关闭。点开后按钮变亮，关闭面板后恢复未选中状态。

左键点形象会立即显示固定回复「喂，戳我干嘛!」，不会再额外请求一次模型回复。右键点形象
会弹出退出确认；确认后桌宠和设置窗口立即消失，后端留在后台完成本场蒸馏、会话落盘和连接
关闭，收尾完成后自行退出，不再让用户停在「正在退出」界面等待。点取消、弹窗外部或按 Esc
都不会退出。

系统页里的用户配置都不要求手动重启。语音输入、语音播报、房间号和主播昵称是本场状态：
修改后立即生效但不写入 profile，下次启动分别恢复为开、开、空、空。空昵称在模型 Prompt 内
自动映射成“主播”。环境噪音灵敏度、事件回复开关、直播主题简介、话痨程度、回复长度、弹幕
整理间隔、电池礼物门槛和进房欢迎人群会在校验通过后立即应用，并写回当前 profile。主播昵称
点输入框右侧的「保存昵称」，直播主题简介点下方的「保存直播主题」；输入过程中不会拿半截文字
刷新模型，保存后下一次回复起生效。
写盘或热更新失败时会回滚原值，
界面显示人话错误。房间号改变时旧连接会释放并接入新房间；「断开连接」只断直播间事件流，
不改麦克风，也不清空已填的房间号。事件流水会展示平台实际给到的昵称、UID 或匿名哈希、用户
等级、财富等级、房管、舰队等级和粉丝牌；不同事件的上游字段并不齐，缺失项不显示。密钥永远
不进面板或 toml。

礼物分档用 B 站前台的电池单价：协议 `price=100` 是 1 电池，礼物总值是单价乘数量；盲盒按
原盲盒价格，不按开出的礼物标价。默认中额从 100 电池起，高额从 1000 电池起。事件流水保留
原始金额和电池数用于核对分档，但模型提示和单轮回复都明确禁止口播金额、电池数或价格。
关注、点赞、分享和房间状态只记录，不生成回复；前三项在设置页固定为未勾选并置灰。

进房身份直接读本次 `INTERACT_WORD_V2`：舰队用户，或佩戴本房等级不低于 5 的粉丝牌用户，
进入按名欢迎通道；历史送礼不再被当成 VIP 身份。普通用户、舰队用户、高等级粉丝牌用户三个
进房开关默认都开，关闭某组只阻止回复，事件流水仍会保留。普通欢迎不会播报进房人数，会从
直播主题简介和本场最近上下文里提取当前内容，每次换一种说法。舰长、提督、总督和本房五级
以上粉丝牌使用不同的欢迎强度；等级来自当前平台事件，不从送礼记忆猜。

弹幕回复会先用一句短转述说明「谁问了什么」，再用 Mia 自己的知识和判断回答。只有主播未公开
计划、私事、个人承诺等确实无法确认的内容，才明确说未知并请主播补充；不会把一般技术问题
统一推给主播，也不会反复说「我只是伴播」或「我可不敢」。

助手市场只保留 Mia，形象固定为豆腐机器人，音色沿用当前 Mia 配置。点 Mia 卡片后可以在
「默认人设」和「改动人设」之间选择；选择会立即刷新当前模型上下文，不关闭或重建语音连接。
每套人设都有独立的 identity 和 personality，只有检测到文本变化时保存按钮才可用；切换方案
和保存修改都会先弹出二次确认，确认后立即刷新上下文。启动默认选改动人设；它读取
当前内容与 `config/personas/live/identity.md`、`personality.md` 同步，并保持 Mia 的形象和音色。
聊天记录里的助手回复会引用触发它的弹幕、礼物、SC 或进房事件，方便逐条
对照；主播语音触发的隐式回复没有直播事件引用。
...
- 界面本身不播放伴播声音；声音仍由 dev-talk 进程输出。常规页面不采集任何输入，只有你在
  「直播 Mock」里主动选择共享源后，所选浏览器音轨才会替代麦克风送入 dev-talk。
...
```bash
.venv/bin/bilisama persona list        # 随包人设：mia、live + openhanako 移植的 hanako/ming/butter
...
**人设变量**：人设文件里的 `{{username}}`、`{{agentName}}` 和 `{{replyLength}}` 由配置填值；
旧模板使用的 `{{userName}}` 仍作为兼容别名保留。
系统页修改「主播昵称」后点「保存昵称」，修改「直播主题简介」后点「保存直播主题」，都会立即
刷新模型上下文，不需要重启。昵称只管本场，下次启动输入框重新留空；直播主题会写回配置。
空昵称在 Prompt 中按“主播”处理：

```toml
[persona]
streamer_name = ""          # 面板每次启动清空；空值在 Prompt 中按「主播」处理
display_name = ""           # 它自称什么，留空用人设的目录名

[room]
stream_intro = "今晚从零搭一个 ComfyUI 工作流"  # 当前直播主题，最多 500 字
```
...
`{{replyLength}}` 跟系统页的「回复长度」共用 `interaction.reply_length`：low 对应 45 token，要求
只说一个短句且通常不超过 20 个汉字；medium 对应 120 token 和通常一到两句，high 对应
180 token 和通常两到四句。
直播中切换会同时更新模型的 token 上限和人设里的口语长度说明，下一条回复起生效；具体事件
若要求更短（例如批量进房欢迎固定一句），以事件限制为准。
...
- 目录名跟当前人设走（mia/live/hanako/ming/butter），每个人设的备忘各自独立。
...
`dev-talk` 没有 `--mic` 参数；不传 `--wav` 时本来就是麦克风模式。`--room` 只增加直播事件
源，语音仍来自 `--input-device` 指定的本机输入设备，不会自动拉取 B 站播放器音轨。主播本机
联调用主播麦克风；旁路机要听远程直播，需自行用虚拟声卡把播放器音频路由为输入，并把 AI
输出送到另一设备，避免回声自激。
...
- 礼物连击结算成一句谢（空闲 1 秒结算，同一组合 10 分钟内不谢第二次）；100/1000 电池是
  默认中额/高额门槛，前台可热调整
...
这条路径不换后端：Qwen realtime、Assembly、弹幕筛选、调度、记忆和输出仍是平时启动的
同一套。界面只把语音输入从电脑麦克风切到共享浏览器的音轨，并按你填的房间号连接
`BilibiliEventSource`。共享画面只用于确认选对了页面；当前音频模型看不懂画面内容。

直播 Mock 检测成功后，它填写的房间号、直播主题简介和五类事件开关会覆盖系统页配置；直播
主题简介和事件开关写回当前 profile，房间号只管本场。换了房间但没有新主题时，简介会清空，
系统页旧房间的事件流水也会清空。旧房间
连接会在这时释放，停止 Mock 后连接刚保存的新房间，因此两条真实房间流不会同时向调度器喂
事件，也不需要重启。照常启动：
...
1. 打开桌面面板「测试」，点最上方「直播 Mock → 打开」，macOS 会直接用 Chrome 打开。也可以给
   启动命令加 `--open`，在浏览器页面里进入。要稳定捕获单个 B 站标签页的声音，请用 Chrome。
2. 点「选择共享源」，选正在播放直播的浏览器标签页，并勾选「共享标签页音频」。页面必须
   同时看到画面预览和「音轨已收到」；只有画面、没有音轨不能通过检测。
3. 输入 B 站房间号和本次直播主题简介。短号可以；更换房间时主题默认清空，避免把旧房主题
   带进新房间。检测成功后会显示解析出的真实房间号和当前人气。
4. 在「参与伴播的真实事件」里选择哪些事件可以触发说话。这组选择会覆盖系统页对应开关；
   关闭某项只阻止它触发回复，原始事件仍进入记忆与监控。
5. 点「检测状态」。伴播后端、共享画面、共享音轨、真实直播间流四项都绿，才会解锁
   「开始直播 Mock」。检测阶段收到的房间事件只证明连接正常，不进入伴播调度。
6. 点「开始直播 Mock」。从这一刻起，电脑麦克风帧会被丢弃，共享音轨下采样成 16 kHz
   单声道 PCM 后送进现有 SpeechLink；真实直播事件送进现有 Assembly。下方监控会显示
   房间事件、浏览器语音转写和伴播回复。
7. 点「停止 Mock」会停止向伴播发送共享音频和房间事件，并恢复电脑麦克风。点「停止共享」
   还会释放屏幕与音频轨道。

macOS 首次使用时，Chrome 会请求屏幕录制权限。共享 B 站标签页时必须勾选标签页音频；
检测页会阻止只有画面、没有音轨的共享源开始运行。

这条验证链路会接真实直播事件，但 AI 输出默认仍只在本机播放，不会自动进入 OBS 或推流。
因此可以先做影子验证；是否把伴播声音送进直播软件，仍由你的音频路由决定。

## MVP 验证测试集

启动 BiliSama 后打开桌面面板的「测试」页。这里有两套可以逐条运行的用例：

- 「功能验收」20 条，覆盖语音连续对话、打断、键盘噪声、BGM、弹幕选择、伴播自主回答、
  普通欢迎、四档重点观众身份、礼物、SC、三档上舰、冷场、安全、观众记忆、优先级和暂停恢复；
  所有外部直播事件种类都覆盖，VIP 由真实进房身份提升，不直接伪造内部 `vip_enter`。
- 「业务测试」36 条：AI代码侠土豆、BetterGI、清月已经不困了、老师好我叫石同学、
  Ai_随风、小小小名不是小明各 6 条。选定主播后按 `01` 到 `06` 运行，依次覆盖开场、
  主播忙碌时的问答、冷场或内容承接、付费互动、业务边界、压力或跨场回访。切到业务
  测试时默认只显示第一位主播的 6 张卡，不会一次铺开 36 张；下拉框可以换人或查看全部。

点「运行」后，页面列出的 mock 事件会按时间送进正式 Assembly，和真实直播事件走同一套
记忆、筛选、调度和说话开关。一次只运行一条；随时可以点「停止当前」。显示“事件已注入”
只代表脚本执行完，不代表效果通过——对照卡片里的“通过标准”听完后，点「通过」或「失败」
记录本轮人工判断。语音、打断、噪声和暂停总闸用例还需要按“你要做”配合说话或点按钮。

压力和边界不再单独放一组抽象卡：BetterGI 的「Issue 洪峰」会在一秒内注入 120 条报错弹幕
并夹两条 SC；石同学的「演示出圈」会在两秒内注入 200 名观众进房；Ai_随风的「成片支持」
会展开 20 帧礼物连击；清月的付费卡包含 SC 撤回；小明的最后两张卡要连续运行，验证老观众
跨场回访。页面只显示一条“洪峰 × 数量”摘要，runner 仍会逐条送进 Assembly。洪峰卡从
归一化事件层起跑，能验证筛选、合并、调度、UI 是否卡顿和语音是否连续，但不验证 B 站原始
包解析预算；那一项仍由底层自动回归和真实房间影子运行负责。

两套运行数据的唯一真相源分别是 `config/testsets/functional.json` 和
`config/testsets/business.json`。改完先跑结构、覆盖和 runner 回归：

```bash
.venv/bin/pytest -q tests/unit/test_ui_test_runner.py
```

完整验收口径、真实房间影子运行、边界/压力矩阵和放行标准仍见
[mvp-validation-plan.md](mvp-validation-plan.md)。UI 里的两套回放只生成虚构直播行为；语音由测试人
真实对着电脑麦克风说。真实房间先做 AI 不进 OBS 的影子运行，通过稳定性和付费事件对账后，
再让声音进入直播。
...
主动话题的当前取材顺序是：本场最近 12 条已完成的主播/Miya 对话、最近 20 条直播事件、
以及由直播事件滚动生成的「本场进展」。对话历史只保存在当前进程内，每条最多 200 字；
被打断、失败或超时的助手半句话不会进入取材，避免把主播没真正听完的内容继续往下聊。
