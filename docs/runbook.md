...
从这里切人设会同时清掉 `display_name` 的全局覆盖，她会用新人设自己的名字自称——
命令行 `--persona` 临时切换没有这个待遇，见下面「名字」一节的老账 #86。
...
⚠️ `display_name` 是全局的，不跟着 `--persona` 走：出厂填着「豆腐」，所以命令行
`--persona` 切到 hanako / ming / butter 之后她仍然自称豆腐，换人设时这一行要跟着改。
从面板助手页切换没有这个坑——它会同批把 `display_name` 清空，让新人设用自己的名字。
老账 #86 剩下的就是命令行这条路。
