...
系统 / 聊天记录 / 助手 / 测试 / 运行日志 / 高级，顶栏是紧急叫停按钮和连接胶囊）。
桌宠旁边竖排四个快捷键——语音输入、语音播报、暂停、设置——悬停有提示，选中态
跟面板里的开关同步；右键形象弹出退出确认，确认后壳和后端一起收尾。面板里的注入框
和终端打字完全等价，`/sc` `/gift` 语法通用。点击形象有动画反应，偶尔她还会开口
回一句（15 秒冷却，说话权闸门空闲才说，不会打断正事）。气泡有个分流规则：语音输入和播报都开着时，
她的语音回复和礼物、SC、上舰、VIP 这些高价值回复才冒气泡，普通弹幕回复只出声不冒泡
...
| 不碰什么 | 麦克风照常收音，事件照常进队 | 记忆和面板照常——暂停不是失忆，事件仍然记录，只是不发声、不上行 |
...
- **声音设备区**：麦克风和扬声器按设备名选、一键试音（从原「现场」页搬来）。
  ...
  起播缓冲防网络抖动把词切碎，而且会每秒跟随 macOS 默认输出设备——中途连上蓝牙
  耳机会自动切过去（显式 `--output-device` 则钉住不动）；这两样的健康计数在高级页
  健康卡的 `audio_output` 一张里。
- **直播间配置卡**：房间号加「检测连接 / 断开」（进程内热切换，见下面「直播间热切换」）、
  主播昵称、直播主题简介；卡底下是这一场的直播间事件流水，换房号时清空。
- **互动卡**：各事件说话开关、话痨度、回复长度、礼物双门槛（单位是电池）、进房欢迎
  开关、DashScope 判停参数（改完自动重连生效）。
- **手动事件 Mock**：这里注入的弹幕走真实挑选漏斗（打分、开窗、预算），跟终端直敲的
  「敲什么答什么」不是一条路。

改配置的回执分两种：**「已保存并生效」**是写回了 `bilisama.toml`（保留注释，写进当前
active profile）；**「本场已生效，重启恢复默认」**是会话级开关（音频双开关、房间号、
...
换房不用重启进程：系统页「直播间配置」卡填新房号点「检测连接」，连上了就开始收
...
getDisplayMedia 这件事壳里做不了，所以走外部浏览器。流程照页面上的三步走：

1. Chrome 里先开着要模拟的直播间播放页。
2. 控制台点「选择共享源」，选那个标签页，**勾上「共享标签页音频」**——共享源没有
   音轨过不了预检。
3. 填真实房间号点「检测状态」：后端会连上房间，但事件先不进产品链路，四项检查
   （伴播后端 / 共享画面 / 共享音轨 / 直播间流）全绿才算就绪。
4. 点「开始 Mock」：浏览器音轨顶替麦克风上行（同一条音频通道、同样的 16k PCM），
   真实房间事件同时进正式链路，她的回复照常从壳或页面出声。

「停止」或浏览器结束共享都会把输入切回麦克风，预检状态同时作废，重来要重新检测。
...
每 5 秒 fetch 的就是它（`panel.js:140`，只在面板开着时轮询）。
...
| director 打字「没反应」 | 按顺序看：有没有 `[已注入 弹幕]` 回显（没有＝输入没进来）→ 有没有 `[调度] danmaku → …` 结论（`expired@queued`＝排队超过 20 秒有效期，多半是说话权一直没放开。先看「高级」页那张 `echo` 卡：她要是在听自己说话，就有东西在回声消除够不着的地方放她的声音，最常见是 OBS 的「监听并输出」。走本机那条老路的场次才轮到戴耳机或 `--mute-while-speaking`。另外普通互动共用一个令牌桶预算——话痨度决定容量 1/2/3，刚说完几句时新弹幕要等预算回来，礼物 SC 上舰不受它管） |
