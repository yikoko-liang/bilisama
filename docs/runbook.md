...
- **主播打字回观众**：主播带 @ 目标回复（平台 `reply_mid` / `reply_uname`）时，程序把这位观众最近那条弹幕标成已处理，从回复队列和延迟池撤掉。
  bilibili 对普通观众的 uid 打了码（uid 0，只有 uid_hash），主播回复里带的却是对方真实 mid，按 uid 对不上；2026-09-17 起 uid 被打码的观众
  按回复里的目标昵称匹配（意图测试台也是这样打码的，hard-19 之前因此一直没拦住）。
- **观众互聊只按事实判，程序判**（2026-09-17 起）：一条观众弹幕是互聊，当且仅当 ① 它 @ 了别的观众（平台 reply 目标是
  别的观众，或正文以「@昵称 」开头且昵称不是主播也不是她），或 ② 发它的人 90 秒内刚被别人 @ 过；两种情况下正文点了主播、
  她或「大家」这类面向全场的词都不算。互聊直接不进回复队列，记忆和面板照常有；日志 `assembly.viewer_chat_skipped reason=at_viewer /
  thread_reply`，健康卡 assembly 里 `viewer_chat_skipped` 和 `viewer_chat_reasons`。其它观众弹幕一律正常处理，模型不再有
  「我觉得这是互聊」的裁量：事件规则里只剩「共享记录标明已处理且无补充」和「主播要求静默」两个 `[SKIP]` 理由；观众回应主播
  提问或观点的弹幕按正常弹幕整理给主播（hard-09 那种），主播在忙不是理由。规则在 `src/bilisama/director/viewer_threads.py`，
  接线在 `app.py:on_event`；主播本人弹幕的那条路（只写上下文、关闭 @ 目标）没动。误拦看日志里的 reason 和 thread_note。
...
事件回合的规则里不再让模型判断"主播现在有没有在说话"（2026-09-16）。播放时机从来是程序的事：主播在说就闸住，说完再派发，
模型看不见地板空没空，让它判断只能拿历史里最新那轮当"现在"——实测一条舰长进房被插话后重放，模型看到上一轮是主播讲解，
就写了 `[SKIP] 主播正在解释设定`。现在事件 prompt 直接讲分工：什么时候播由程序决定，只管生成；判断一条事件要不要回，
只看这条事件和共享处理记录里它的状态，主播最近讲什么、她上一轮说什么，只用来措辞，不是跳过的理由。被插话后重新入队的
SC / 礼物 / 上舰 / VIP 重放（2026-09-17 改）：不再把原指令原样再发一遍。重放是另一种输入：写一条「[重放] 你对 X 的回复被主播插话打断，你说到「…」，主播现在说完了……」条目，
指令只留这类事件的形状规则，把通用判断块、"只评估候选记录"和报告规则那一节都去掉，换成 `REPLAY_RULES`：只判一件事，插话这段主播有没有亲口把这件事处理了，
处理了就 `[SKIP] 主播已处理` 或附和一句，没处理就接着说，这轮不调用 `report_interaction`。之前原样重发时模型把这轮当成"重新审一遍旧事件"，三次都只发了空报告不说话
（`spoken@generating` 且无文本）。同一意图最多重放两次，第三次判 `cancelled@speaking(scheduler.replay_limit)`；completed 却没文本没音频的回合判 `failed@generating(model.empty_reply)`，
日志 `scheduler.empty_reply`。

主播说「帮我看下弹幕」「整理一下弹幕」时，语音回合以 `[SUMMARY]` 记号开头（2026-09-15 起；此前走
`danmaku_summary.action=start` 函数报告，真实模型 9 月 14 日的探测里必报组 0/10，所以改成和 `[SKIP]` 同一条
记号通道，四家后端都认，不依赖 DashScope 的工具报告）。语音门解出记号后这一轮照常静音，
`dev_talk.on_voice_skip` 调 `Assembly.request_danmaku_summary`，把门记下的这轮主播语音开始时刻作为上界，
立即从此前收到且尚未处理的最新弹幕候选中建立一次 `danmaku_summary` 意图；它按 90 档排在主播语音之后、
SC 之前，因为产物是给主播听的语音回复。函数报告里的 `start` 仍被接受（同一份候选已消费时不会重复交付），
但提示词不再要求它；`cancel` 照旧。日志看 `voice_gate.skipped category=summary` 和
`dev_talk.summary_delegated delivered=…`。模型需要从候选里选出最近仍在热议的一个话题再总结，
而不是逐条复述。语音期间或之后才到的弹幕不混入总结，也不会因为这项委托再次重复普通回复。
验证时先注入多条围绕同一主题的弹幕和一条孤立弹幕，再说出委托，检查模型输入包含候选但口播只围绕热议主题；随后注入的新弹幕不应再次触发这次总结。

总结的候选只排除两种：主播本人处理过的（语音或手动回复标了已处理的），和上一次总结已经交付过的；她自己答过的算，行尾标「你已回过」，主动话题的记账和这条车道无关，她要交代谁问了什么、自己怎么答的（2026-09-17：主播问「我不在时大家问了你什么」，被答过的小满那条不能因为答过就不见）。委托找不到可总结的弹幕时她不再沉默（2026-09-16 起，`dev_talk.summary_delegated outcome=empty`）：主播交代了事，
沉默不是答案，她会用一句「刚才没有新弹幕」回掉；半分钟内刚交付过一次总结、又来一次同样的委托（函数报告重复记号那种）
才真的不出声（`outcome=silent`）。没形成热议话题时也一样要回主播一句，说各问各的、点一两条值得看的，而不是 `[SKIP]`。
意图测试台的边界（2026-09-17 改）：进一个用例（`begin_test`）把主动话题的素材池、账本、对话环全部清空，
出用例（`end_test`）只撤掉用例背景，欠着的回复、没人答的弹幕、账本和小时预算都留着——之前出用例也清，
面板上看到的就是"她回糯米回到一半被打断，用例一结束，下一个主动话题却去聊趣味池"。

「帮我看下弹幕，大家有什么问题？」这种后半句像在问观众的说法，模型今天三次都判成 `[SKIP] 主播在面向观众提问`：语音规则里加了一句
后半句是在描述要看的内容、仍是 `[SUMMARY]`，例子也补了；程序再兜一层，主播这轮转写里出现「看下/整理/总结…弹幕」这类字面而模型写了
`[SKIP]`，按委托处理（`scene_markers.looks_like_summary_delegation`，转写晚到 3 秒内也认），日志 `voice_gate.summary_by_text`。

同一原则写进了语音规则（`voice_addressing.md`「交代助手与面向观众」）：主播叫着她的名字交代一件事，说得清不清楚都要回，
听清了就做，没听清就问一句，点了名的交代不能 `[SKIP]` 先听。
...
collection_window_s = 30
```

默认 30 秒（2026-09-17 从 120 改），范围 5～1800 秒。计时起点应是主播开始征集的语音，不能用后台报告晚到的时刻。
...
### 主动话题从哪来、为什么不会翻来覆去说一件事

2026-09-16 起主动话题按七层来源选（`src/bilisama/proactive_sources.py`），每次开口只取最靠前一层有素材的：
1 欠着的回复（被打断没说完的）→ 2 真没人答的观众弹幕 → 3 从最近几条弹幕里抽的可讨论话题 → 4 接主播刚说的话
→ 5 本场进展 / 直播简介 → 6 在场常客的记忆（一人一场最多点名两次）→ 7 趣味池。房间越热放行的层越少：
繁忙不起题，活跃只到第 4 层，稀疏加第 5 层，冷清全开。日志 `proactive.topic_submitted` 的 `layer` 字段就是这次走了哪层。

**趣味池**是冷场兜底：`config/prompts/topics/` 下每个 `.md` 一行一条轻问题（`#` 开头是注释），出厂带
`aigc.md`（AI 画图教学场）和 `coding.md`（AI coding 场）各 26 条。`interaction.proactive.topic_pool` 填文件名
（不带 `.md`，逗号分隔）选用哪些，留空全用；改动重启生效。一场里每条只用一次，每次随机抽三条让她挑最贴的一条，
所以池子里的话不会原样念出来。加自己的场：新建一个 `.md`，写二三十句面向整个直播间的低门槛问题。

欠着的回复被一次主动开口"拿走"后，那次开口没播出去（过期、被抢、开口前被切）会原样还回来，播出去了才算了结；
被主播说过头的她自己的语音片段（"太好了，终于…"）排在回观众的后面，池子满了也先挤掉这些。

第 1 层"欠着的回复"两种打断都算：她话说到一半被主播插话（生成被打断，`cancelled@speaking`），和她已经说完、
音频放到一半被插话（播放被打断，`spoken@generating`，观众只听到开头）。素材里会标明是哪一种，
后者给的是全文，让她接着说而不是从头再来。

**为什么现在不重复了**——三道：
- 普通车道回过或判过不值得回的弹幕，判决落地时就标成「已回答」（`Assembly.note_verdict`），之后永远不当「没人答的」，
  最多在第 3 层带「[已回答，不要再答]」标注出现。以前这一步缺着，是「她把我十分钟前问过的又当话题起了」的根子。
- 本场发起过的话题记账（`TopicLedger`，1 小时窗），记的是她实际说出口的话。侧路模型给的候选跟账本撞了
（相似度 ≥ 0.6）直接丢掉（`proactive.candidate_duplicate`），改由实时模型按层素材起；说出口之后才发现重复的
（音频已播，收不回）记 `proactive.repeat_detected`，冷却 5 分钟。账本同时写进她的指令里。
- 两次主动开口至少隔 90 秒，连续两次没人接翻倍，三次后等房间有新事件；主播说话和她自己说完都重置冷场计时
（以前主播独白期间计时不停，她会在主播刚停九秒时接话）。

面板「聊天记录」里主动话题那条的引用行写着走了哪层：「引用 主动话题 · 第 2 层·没人答的弹幕·侧路候选」，尾巴是「侧路候选」还是「实时自选」说明侧路模型的候选有没有赶上（`dev_talk.py` 里从 `proactive.opening_info` 取）。排查：`proactive.no_material` 是「这个档位一层都没素材，所以没起」，冷清房不会出现这条（会退到实时模型自己起一句）；
面板 health 的 proactive 卡有 `last_layer`、`recent_topics`、`repeats_detected`、`candidates_dropped` 和池子余量。
