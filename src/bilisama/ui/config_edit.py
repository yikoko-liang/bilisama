...
def apply_config_edit(settings: Settings, path: Any, value: Any) -> tuple[FieldMeta, Any]:
    ...
    return _apply_config_edit(
        ...
        path,
        value,
        allowed_reload=frozenset({Reload.LIVE}),
    )
...
def apply_runtime_config_edit(settings: Settings, path: Any, value: Any) -> tuple[FieldMeta, Any]:
    ...
    The caller must run the matching hook before acknowledging the edit. An
    engine-level change remains excluded because dev-talk cannot replace the
    separate local speech engine safely inside one process.
    """
    return _apply_config_edit(
        settings,
        path,
