...
// PCM rides its own point-to-point binary socket (?role=mock), exactly like
// the microphone path — never the broadcast control socket. The shipped
// capture-worklet already resamples to 16 kHz Int16 20ms frames, so the tab
// track and the microphone produce byte-identical shapes.
...
function openAudioSocket() {
  ...
  audioSocket = new WebSocket(`ws://${location.host}${tokenRoot}/audio?role=mock`);
...
async function startAudio(selected) {
  ...
  audioNode.port.onmessage = (message) => consumeFrame(message.data);
...
function consumeFrame(buffer) {
  ...
  if (audioSocket?.readyState === WebSocket.OPEN) audioSocket.send(buffer);
  else openAudioSocket();
}
...
function runCheck() {
  ...
  if (streamIntroEdited) {
    ...
    configuredStreamIntro = streamIntro.value.trim();
  }
