...
export function createPanel({ send }) {
  ...
  const renderTestSteps = (card, item) => {
    ...
    card.appendChild(el("p", "test-classification", "七类实际分类：当前后端未提供；当前接话/先听判断见系统页和聊天记录，沉默不代表识别正确。"));
  ...
  const renderTestObservations = (container, state) => {
    ...
    for (const observation of observations) {
      ...
      row.appendChild(el("p", "test-classification", "七类实际分类：当前后端未提供；接话/先听结果见系统页和聊天记录。"));
