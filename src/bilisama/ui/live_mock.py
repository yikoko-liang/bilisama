...
The controller changes inputs, not product behaviour. Browser PCM still enters
the existing ``SpeechLink`` and room events still enter ``Assembly.on_event``.
Preflight keeps both lanes out of the product path until the operator starts a
run, which makes a green checklist mean something observable.
"""
...
class LiveMockController:
        ...
        return {
            ...
            "checks": {
                ...
                "audio": {
                    ...
                    "label": "共享音轨",
                    "detail": (
                        f"{sample_rate_hz} Hz，运行时转为 16000 Hz PCM"
                        if audio_ok
    ...
    async def push_audio(self, data: Mapping[str, object]) -> None:
        """Decode one browser frame; malformed or stale frames are accounted, not raised."""
        if not self._running or self._status != "running":
            self._audio_dropped += 1
            return
        payload = data.get("pcm16_b64")
        if data.get("sample_rate") != _INPUT_RATE or not isinstance(payload, str):
            self._audio_dropped += 1
            return
        try:
        ...
        if self._audio_frames == 1 or self._audio_frames % 10 == 0:
            self._publish()
