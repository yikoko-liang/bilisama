...
class SwitchableRoomSource:
    ...
    async def start(self, emit: EventSink) -> None:
        while not self._stopped:
            ...
            # Consume the signal that woke this build: leaving it set makes the
            # in-flight wait() below complete instantly, which reads as "the
            # room changed again" and rebuilds a source that just failed.
            self._changed.clear()
    ...
    def logged_in(self) -> bool:
        """Delegates to the live transport, for the credential watcher."""
        return bool(self.status().get("logged_in"))

    @property
    def credential_stale(self) -> bool:
        return bool(self.status().get("credential_stale"))
