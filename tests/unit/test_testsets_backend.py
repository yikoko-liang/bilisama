...
QueueSource to the real Assembly, selector, presence aggregation and, for two
representative collision/pressure cases, the real Scheduler and SpeechLink.
"""
...
_TEMPLATE = _ROOT / "config" / "personas" / "mia"
...
def _thresholds(interaction: InteractionConfig) -> Callable[[], DerivedThresholds]:
    def read() -> DerivedThresholds:
        return effective_thresholds(
            interaction.chattiness,
            reply_length=interaction.reply_length,
            danmaku_window_s=interaction.danmaku.window_s,
        )
...
def _assembly(
    ...
    *,
...
) -> tuple[Assembly, DanmakuSelector, MemoryStore]:
    ...
    selector = DanmakuSelector(
        clock,
        thresholds=_thresholds(interaction),
        per_uid_cooldown_s=float(interaction.danmaku.per_uid_cooldown_s),
    )
    ...
    assembly = Assembly(
        ...
        presence=PresenceWelcomer(
            uniques=interaction.burst_uniques,
            window_s=float(interaction.burst_window_s),
            cooldown_s=float(interaction.burst_cooldown_s),
        ),
    ...
    return assembly, selector, store
...
async def test_every_case_enters_the_real_assembly_and_selector(
    ...
    """All 54 cards execute against the same components dev-talk wires."""
    clock = FakeClock(wall=datetime(2026, 8, 22, 20, 0, tzinfo=UTC))
    interaction = _interaction()
    intents: list[Intent] = []
    observed: list[LiveEvent] = []
    floor = SpeakingFloor(clock)
    assembly, selector, store = _assembly(
        tmp_path,
        clock,
    ...
    try:
        ...
        # Close a default danmaku window and settle any gift combo.
        await clock.advance(interaction.danmaku.window_s + 2)
        ...
        await asyncio.gather(source_task, selector_task, return_exceptions=True)
        store.close()
...
async def test_representative_function_and_business_cases_reach_the_real_scheduler(
    ...
    async with MockRealtimeServer(
        ...
        try:
            ...
            await clock.advance(case.duration_s + interaction.danmaku.window_s + 2)
            await _wait_until(
            ...
            await runner.stop()
            await source.stop()
            for task in (source_task, selector_task, scheduler_task):
                task.cancel()
