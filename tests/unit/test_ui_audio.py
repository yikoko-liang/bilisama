...
async def test_explicit_test_audio_is_recorded_after_the_shared_gates(tmp_path: object) -> None:
    from pathlib import Path

    from bilisama.ui.audio import AudioInputSwitch, UplinkRecorder

    assert isinstance(tmp_path, Path)
    out = tmp_path / "test-uplink.wav"
    sent: list[bytes] = []
    ...
    await switch.push_test_audio(_loud_frame())
    recorder.close()
    with wave.open(str(out)) as recorded:
        assert recorded.readframes(recorded.getnframes()) == b"".join(sent) == _loud_frame()
